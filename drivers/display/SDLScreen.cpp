...
/**
 * SDL driver for high-res OLED displays
 */
...
#define SDLPANEL_W 1280
#define SDLPANEL_H 480

//
//	Init the Virtual display driver
//
void SDLScreen::init(const char *param) {

	panelW = SDLPANEL_W;
	panelH = SDLPANEL_H;

	printf("*Init SDL high-res panel driver\n");
	...
	if(p) {
		...
		printf("*SDL display set width to %d\n",panelW);
	}
	...
	if(p) {
		...
		printf("*SDL display set height to %d\n",panelH);
	}
	...
	win = SDL_CreateWindow("",0,0,panelW,panelH,SDL_WINDOW_RESIZABLE);
